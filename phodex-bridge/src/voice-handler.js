...
async function resolveVoiceAuth(sendCodexRequest) {
  ...
  try {
    ...
    console.error(`[mobidex] voice/resolveAuth: getAuthStatus RPC failed: ${err.message}`);
  ...
  if (!token) {
    console.error(`[mobidex] voice/resolveAuth: no token. authMethod=${authMethod || "none"} requiresOpenaiAuth=${authStatus?.requiresOpenaiAuth}`);
