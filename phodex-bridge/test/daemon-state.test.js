...
test("daemon-state stores config, pairing payloads, and status under the mobidex state dir", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    assert.equal(resolveMobidexStateDir(), rootDir);
...
function withTempDaemonEnv(run) {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "mobidex-daemon-state-"));
