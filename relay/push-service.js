...
// Purpose: Stores session-scoped APNs registration state and sends completion pushes for Mobidex sessions.
...
function apnsConfigFromEnv(env) {
  return {
    teamId: readFirstDefinedEnv(["MOBIDEX_APNS_TEAM_ID", "REMODEX_APNS_TEAM_ID", "PHODEX_APNS_TEAM_ID"], env),
    keyId: readFirstDefinedEnv(["MOBIDEX_APNS_KEY_ID", "REMODEX_APNS_KEY_ID", "PHODEX_APNS_KEY_ID"], env),
    bundleId: readFirstDefinedEnv(["MOBIDEX_APNS_BUNDLE_ID", "REMODEX_APNS_BUNDLE_ID", "PHODEX_APNS_BUNDLE_ID"], env),
    privateKey: readAPNsPrivateKey(env),
  };
}

function readAPNsPrivateKey(env) {
  const rawValue = readFirstDefinedEnv(["MOBIDEX_APNS_PRIVATE_KEY", "REMODEX_APNS_PRIVATE_KEY", "PHODEX_APNS_PRIVATE_KEY"], env);
  if (rawValue) {
    return rawValue;
  }

  const filePath = readFirstDefinedEnv(
    ["MOBIDEX_APNS_PRIVATE_KEY_FILE", "REMODEX_APNS_PRIVATE_KEY_FILE", "PHODEX_APNS_PRIVATE_KEY_FILE"],
    env
  );
  if (!filePath) {
    return "";
...
function resolvePushStateFilePath(env = process.env) {
  const explicitPath = readFirstDefinedEnv(
    ["MOBIDEX_PUSH_STATE_FILE", "REMODEX_PUSH_STATE_FILE", "PHODEX_PUSH_STATE_FILE"],
    env
  );
  if (explicitPath) {
    return explicitPath;
  ...
  return path.join(codexHome, "mobidex", "push-state.json");
}
