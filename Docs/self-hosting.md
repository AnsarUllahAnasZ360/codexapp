# Self-Hosting Mobidex

This guide is for developers who clone the public GitHub repository and want to run Mobidex on infrastructure they control.
...
- local pairing should work out of the box with `./run-local-mobidex.sh`
- internet-facing setups should pass their own relay URL explicitly with `MOBIDEX_RELAY`
...
## What Mobidex Self-Hosting Means

Mobidex is local-first.
...
- an iPhone with a Mobidex build installed
...
```sh
./run-local-mobidex.sh --hostname 192.168.1.10
```
...
- a shared-domain subpath, for example `wss://api.example.com/mobidex/relay`
...
```sh
MOBIDEX_RELAY="wss://relay.example.com/relay" mobidex up
```
...
cd phodex-bridge
npm install
MOBIDEX_RELAY="wss://relay.example.com/relay" npm start
```
...
If you install the bridge from npm and do not use the local launcher, make sure you export `MOBIDEX_RELAY` before running `mobidex up`.
...
- you do not need push to use Mobidex
...
- a bridge-side `MOBIDEX_PUSH_SERVICE_URL`
...
- only enable `MOBIDEX_TRUST_PROXY=true` when the proxy is trusted and sanitizes forwarded IP headers
...
- the bridge is using the correct `MOBIDEX_RELAY`
...
- point the bridge at your relay with `MOBIDEX_RELAY`
