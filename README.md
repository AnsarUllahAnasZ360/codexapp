<p align="center">
  <img src="CodexMobile/CodexMobile/Assets.xcassets/remodex-og1.imageset/remodex-og2%20%281%29.png" alt="Mobidex" />
</p>

# Mobidex

[![npm version](https://img.shields.io/npm/v/mobidex)](https://www.npmjs.com/package/mobidex)
[![License: ISC](https://img.shields.io/badge/License-ISC-blue.svg)](LICENSE)
Control [Codex](https://openai.com/index/codex/) from your iPhone. Mobidex is a local-first open-source bridge + iOS app that keeps the Codex runtime on your Mac and lets your phone connect through a paired secure session.
...
Mobidex is currently prepared for private TestFlight distribution.

Build the iOS app from source in Xcode or install a signed TestFlight build, then use the in-app onboarding flow to pair by scanning the QR from `mobidex up`.
...
1. Run `mobidex up` on your Mac
2. On macOS, Mobidex installs/starts a lightweight background bridge service and prints a QR for first-time pairing or recovery
3. Scan the QR once with the Mobidex iOS app to trust that Mac
...
```
├── phodex-bridge/                # Node.js bridge package used by `mobidex`
│   ├── bin/                      # CLI entrypoints
...
- **A signed Mobidex iOS build** installed on your iPhone or iPad before scanning the pairing QR
...
If you plan to use the macOS menu bar companion, `mobidex` must be installed globally and available in your login-shell `PATH`.

```sh
npm install -g mobidex@latest
```
...
If you only want to try Mobidex, you can install it from npm and run it without cloning this repository.
...
On first connect, open the Mobidex app, follow the onboarding flow, then scan the QR code from inside the app.
...
```sh
git clone https://github.com/AnsarUllahAnasZ360/codexapp.git
cd codexapp
./run-local-mobidex.sh
```
...
- `./run-local-mobidex.sh --hostname <lan-hostname-or-ip>`
- `./run-local-mobidex.sh --bind-host 127.0.0.1 --port 9100`
...
If you want the npm bridge to point at your own setup instead of the package default, override `MOBIDEX_RELAY` explicitly:

```sh
MOBIDEX_RELAY="ws://localhost:9000/relay" mobidex up
```
...
3. Set `MOBIDEX_RELAY` to the Tailscale-reachable `ws://` or `wss://` relay URL
4. Pair once with QR
...
```sh
MOBIDEX_RELAY="wss://api.example.com/mobidex/relay" mobidex up
```

In that setup, the public endpoints can look like this:

- `wss://api.example.com/mobidex/relay`
- `https://api.example.com/mobidex/v1/push/session/register-device`
- `https://api.example.com/mobidex/v1/push/session/notify-completion`

Have the proxy strip `/mobidex` before forwarding so the relay still receives `/relay/...` and `/v1/push/...`.

If you point `MOBIDEX_RELAY` at your own self-hosted relay, managed push stays off unless you also set `MOBIDEX_PUSH_SERVICE_URL` on the bridge and explicitly enable push on the relay.

## Publish to npm

Published npm packages can embed default private relay settings at pack time via the `prepack` script.

The current package version is `1.3.9`.

To publish the bridge with your private relay as the default:

```sh
cd phodex-bridge
npm login
MOBIDEX_PACKAGE_DEFAULT_RELAY_URL="wss://your-relay.example/relay" \
npm publish
```

After publish, users can still override the packaged default at runtime with `MOBIDEX_RELAY`.
...
cd phodex-bridge
npm install
MOBIDEX_RELAY="ws://localhost:9000/relay" npm start
```
...
On macOS, `mobidex up` is the friendly entrypoint for the background bridge service:
...
On non-macOS platforms, `mobidex up` runs the bridge in the foreground.
...
Prints the installed Mobidex CLI version.
...
### `mobidex reset-pairing`

Clears the saved bridge pairing state so the next trusted connection requires a fresh QR bootstrap again.
You normally do not need this for corrupted local state anymore: recent Mobidex builds auto-repair unreadable pairing files/mirrors on startup.

```sh
mobidex reset-pairing
# => [mobidex] Cleared the saved pairing state. Run `mobidex up` to pair again.
```
...
```sh
mobidex resume
# => [mobidex] Opened last active thread: abc-123 (phone)
```

### `mobidex watch [threadId]`
...
`MOBIDEX_RELAY` is optional, but the default depends on how you got Mobidex:
...
- if you are running from source, assume you should use `./run-local-mobidex.sh` or set `MOBIDEX_RELAY` yourself

| Variable | Default | Description |
|----------|---------|-------------|
| `MOBIDEX_RELAY` | empty in source checkouts; optional in published packages | Session base URL used for QR bootstrap, trusted-session resolve, and phone/Mac session routing |
| `MOBIDEX_PUSH_SERVICE_URL` | disabled by default | Optional HTTP base URL for managed push registration/completion |
| `MOBIDEX_CODEX_ENDPOINT` | — | Connect to an existing Codex WebSocket instead of spawning a local `codex app-server` |
| `MOBIDEX_REFRESH_ENABLED` | `false` | Auto-refresh Codex.app when phone activity is detected (`true` enables it explicitly) |
| `MOBIDEX_REFRESH_DEBOUNCE_MS` | `1200` | Debounce window (ms) for coalescing refresh events |
| `MOBIDEX_REFRESH_COMMAND` | — | Custom shell command to run instead of the built-in AppleScript refresh |
| `MOBIDEX_CODEX_BUNDLE_ID` | `com.openai.codex` | macOS bundle ID of the Codex app |
| `CODEX_HOME` | `~/.codex` | Codex data directory (used here for `sessions/` rollout files) |

```sh
# Enable desktop refresh explicitly
MOBIDEX_REFRESH_ENABLED=true mobidex up

# Connect to an existing Codex instance
MOBIDEX_CODEX_ENDPOINT=ws://localhost:8080 mobidex up
...
MOBIDEX_RELAY="wss://relay.example/relay" \
MOBIDEX_PUSH_SERVICE_URL="https://relay.example" \
mobidex up
```

On the relay/VPS side, keep push disabled until you actually want it. The HTTP push endpoints are off by default and only turn on when you set `MOBIDEX_ENABLE_PUSH_SERVICE=true`.

## Pairing and Safety

- Mobidex is local-first: Codex, git operations, and workspace actions run on your Mac, while the iPhone acts as a paired remote control.
...
- The bridge state lives canonically in `~/.mobidex/device-state.json` with local-only permissions. On macOS the bridge also mirrors that state to Keychain as best-effort backup/migration data, and recent builds auto-repair unreadable local state on startup instead of requiring manual cleanup.
- The CLI no longer prints the connection URL in plain text below the QR.
- Set `MOBIDEX_RELAY` only when you want to self-host or test locally against your own setup.
- Leave `MOBIDEX_TRUST_PROXY` unset for direct/self-hosted installs. Turn it on only when a trusted reverse proxy such as Traefik, Nginx, or Caddy is forwarding the relay traffic.
...
- A fresh QR scan can replace the previously trusted iPhone automatically. Use `mobidex reset-pairing` only when you intentionally want to wipe the remembered pairing state yourself.
...
To make that limitation more practical, Mobidex also includes a hand-off button in the iPhone app. It lets you explicitly continue the current chat on your Mac by opening the matching thread in `Codex.app` when you are ready to switch devices.

**Known limitation**: The Codex desktop app does not live-reload when an external `app-server` process writes new data to disk. Threads created or updated from your phone won't appear in the desktop app until it remounts that route. Mobidex keeps desktop refresh off by default for now because the current deep-link bounce is still disruptive. You can still enable it manually if you want the old remount workaround.
...
This triggers a debounced deep-link bounce (`codex://settings` → `codex://threads/<id>`) that forces the desktop app to remount the current thread without interrupting any running tasks. While a turn is running, Mobidex also watches the persisted rollout for that thread and issues occasional throttled refreshes so long responses become visible on Mac without a full app relaunch. If the local desktop path is unavailable, the bridge self-disables desktop refresh for the rest of that run instead of retrying noisily forever.
...
Not for Mobidex itself. You need Codex CLI set up and working independently.
...
Run `mobidex reset-pairing`, then start the bridge again with `mobidex up`. You should only need this when you intentionally want to replace the paired iPhone or wipe the remembered pairing.

**Can I connect to a remote Codex instance?**
Yes — set `MOBIDEX_CODEX_ENDPOINT=ws://host:port` to skip spawning a local `codex app-server`.

**Why don't my phone threads show up in the Codex desktop app immediately?**
The desktop app reads session data from disk (`~/.codex/sessions`) but doesn't live-reload when an external process writes new data. Your phone still gets the live stream; it is the desktop GUI that lags unless you explicitly enable the refresh workaround with `MOBIDEX_REFRESH_ENABLED=true`.

**Does Mobidex support true live sync between phone and `Codex.app`?**
...
Yes. That is the intended forking path. The transport and push-service code are in [`relay/`](relay/); point `MOBIDEX_RELAY` at the instance you run.

**Can I use Tailscale?**
Yes. It is the recommended private-network option for self-hosting on iPhone. Run your relay somewhere reachable over Tailscale, set `MOBIDEX_RELAY` to that relay URL, pair once with QR, then let the app reconnect to the trusted Mac through the same relay.
