...
private let qrScannerBridgeUpdateCommand = "npm install -g mobidex@latest"
...
func validatePairingQRCode(_ code: String, now: Date = Date()) -> QRScannerPairingValidationResult {
    ...
    if looksLikeMobidexPairingPayload(data) {
        return .bridgeUpdateRequired(
            makeScannerBridgeUpdatePrompt(
                message: "This QR code looks like it came from an older Mobidex bridge. Update the npm package on your Mac to the latest release before scanning a new QR code."            )
        )
    }

    return .scanError("Not a valid secure pairing code. Make sure you're scanning a QR from the latest Mobidex bridge.")
}
...
private func looksLikeMobidexPairingPayload(_ data: Data) -> Bool {
...
private func makeScannerBridgeUpdatePrompt(message: String) -> CodexBridgeUpdatePrompt {
    CodexBridgeUpdatePrompt(
        title: "Update Mobidex on your Mac before scanning",        message: message,
