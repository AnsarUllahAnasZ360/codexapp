...
final class CodexGPTAccountTests: XCTestCase {
    ...
    func testRefreshBridgeVersionStatePresentsOptionalBridgeUpdateWhenLatestIsNewer() async {
        ...
        XCTAssertEqual(service.bridgeInstalledVersion, "1.3.6")
        XCTAssertEqual(service.latestBridgePackageVersion, "1.3.8")
        XCTAssertEqual(
            service.bridgeUpdatePrompt?.title,
            "A newer Mobidex update is available on your Mac"
        )
        XCTAssertEqual(service.bridgeUpdatePrompt?.command, "npm install -g mobidex@latest")
    ...
    func testStartOrResumeGPTLoginOnMacOpensPendingBrowserOnBridge() async throws {
        ...
        service.requestTransportOverride = { method, params in
            ...
            case "account/status/read":
                return Self.loginPendingAccountStatusResponse()
            default:
                XCTFail("Unexpected method \(method)")
                throw CodexServiceError.disconnected
            }
        ...
        XCTAssertEqual(Array(observedMethods.prefix(2)), ["account/login/start", "account/login/openOnMac"])
    ...
    func testHandleGPTLoginCallbackCompletesPendingLoginThroughBridge() async throws {
        ...
        service.requestTransportOverride = { method, params in
            ...
            case "account/status/read":
                return didCompleteLogin ? Self.authenticatedAccountStatusResponse() : Self.loginPendingAccountStatusResponse()
            default:
                XCTFail("Unexpected method \(method)")
                throw CodexServiceError.disconnected
            }
        ...
        _ = try await service.startOrResumeGPTLogin()
        await service.handleGPTLoginCallbackURL(URL(string: "mobidex://auth/gpt/callback?code=abc")!)
        ...
        XCTAssertEqual(
            ...
            "mobidex://auth/gpt/callback?code=abc"
        )
    }
    ...
    func testVoiceTranscriptionReportsDisconnectedInsteadOfLoginWhenBridgeIsOffline() async {
        ...
        await XCTAssertThrowsErrorAsync({
            try await service.transcribeVoiceAudioFile(
                at: URL(fileURLWithPath: "/tmp/mobidex-voice-test.wav"),
                durationSeconds: 1
            )
        }) { error in
    ...
    func testSuccessfulLoginKeepsPollingUntilVoiceTokenIsReady() async throws {
        ...
        XCTAssertGreaterThanOrEqual(accountStatusReadCount, 1)

        if service.gptAccountSnapshot.tokenReady != true {
            XCTAssertNotNil(service.currentPendingGPTLogin())
            await service.refreshGPTAccountState()
        }
    ...
    private static func loginPendingAccountStatusResponse() -> RPCMessage {
        RPCMessage(
            id: .string(UUID().uuidString),
            result: .object([
    ...
    private static func authenticatedAccountStatusResponse() -> RPCMessage {
        RPCMessage(
            id: .string(UUID().uuidString),
            result: .object([
                "status": .string("authenticated"),
